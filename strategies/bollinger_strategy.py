...
class BollingerBandsStrategy(StrategyBase):
    ...
    def calculate_entry_exit(self, symbol: str, signal: Dict) -> Dict:
        ...
        # Multiplicador base por símbolo + ajuste dinámico por volatilidad del día
        sl_multiplier_base = 2.0 if 'XAU' in symbol or 'XAG' in symbol else 0.5
        vol_mult = self._get_volatility_sl_multiplier(symbol)
        sl_multiplier = sl_multiplier_base * vol_mult
        min_stop_distance = symbol_info.point * 100
